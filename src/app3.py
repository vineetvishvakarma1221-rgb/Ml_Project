...
with st.sidebar.expander("ℹ️ How to use this app"):
    ...
    6. Use **📄 Reports** to generate professional PDF/CSV reports.
    """)

# ============================================================
#  TABS
# ============================================================
tab_overview, tab_predict, tab_explain, tab_batch, tab_history, tab_analytics, tab_reports = st.tabs(
    [
        "🏠 Overview",
        "🔍 Live Prediction",
        "🧠 Explainability",
...
# ------------------------------------------------------------
#  TAB: REPORTS — PHASE 5.4
# ------------------------------------------------------------
with tab_reports:
    section_header("📄", "Reports Center", "Generate professional PDF and CSV reports from the current prediction data")

    st.markdown("""
    <div class="feat-card">
        <b>📊 Executive Report</b><br>
        Overall prediction statistics, fraud/normal split, risk distribution and model information.
        <br><br>
        <b>📂 Batch Report</b><br>
        Detailed report for the most recently uploaded and scored customer CSV.
        <br><br>
        <b>👤 Customer Report</b><br>
        Detailed report for the most recent live customer prediction.
    </div>
    """, unsafe_allow_html=True)
    ...
    with exec_col1:
        if st.button("📄 Generate Executive PDF", use_container_width=True, type="primary"):
            try:
                pdf_bytes = generate_executive_pdf(history)
                st.download_button(
                    "⬇️ Download Executive PDF",
                    data=pdf_bytes,
                    file_name=make_report_filename("executive", "pdf"),
                    mime="application/pdf",
                    use_container_width=True,
                    key="download_executive_pdf",
                )
            except Exception as e:
                st.error(f"Executive PDF generation failed: {e}")
    with exec_col2:
        if st.button("📊 Generate Executive CSV", use_container_width=True):
            try:
                csv_bytes = dataframe_to_csv_bytes(generate_executive_csv(history))
                st.download_button(
                    "⬇️ Download Executive CSV",
                    data=csv_bytes,
                    file_name=make_report_filename("executive", "csv"),
                    mime="text/csv",
                    use_container_width=True,
                    key="download_executive_csv",
                )
            except Exception as e:
                st.error(f"Executive CSV generation failed: {e}")
    ...
    if "result_df" in locals() and result_df is not None and not result_df.empty:
        st.success(f"Latest batch result available: {len(result_df):,} customer(s)")
        batch_col1, batch_col2 = st.columns(2)
        with batch_col1:
            if st.button("📄 Generate Batch PDF", use_container_width=True, type="primary"):
                try:
                    pdf_bytes = generate_batch_pdf(result_df)
                    st.download_button(
                        "⬇️ Download Batch PDF",
                        data=pdf_bytes,
                        file_name=make_report_filename("batch", "pdf"),
                        mime="application/pdf",
                        use_container_width=True,
                        key="download_batch_pdf",
                    )
                except Exception as e:
                    st.error(f"Batch PDF generation failed: {e}")
        with batch_col2:
            if st.button("📊 Generate Batch CSV", use_container_width=True):
                try:
                    csv_bytes = dataframe_to_csv_bytes(generate_prediction_csv(result_df))
                    st.download_button(
                        "⬇️ Download Batch CSV",
                        data=csv_bytes,
                        file_name=make_report_filename("batch", "csv"),
                        mime="text/csv",
                        use_container_width=True,
                        key="download_batch_csv",
                    )
                except Exception as e:
                    st.error(f"Batch CSV generation failed: {e}")
    else:
        st.info("Run a batch prediction in the 📂 Batch Prediction tab first. The latest batch result will appear here.")

    st.markdown("---")

    # ---- Customer Report ----
    st.markdown("### 👤 Customer Report")
    if customer is not None and result is not None:
        st.success("Latest live customer prediction is available.")
        customer_col1, customer_col2 = st.columns(2)
        with customer_col1:
            if st.button("📄 Generate Customer PDF", use_container_width=True, type="primary"):
                try:
                    pdf_bytes = generate_customer_pdf(customer, result)
                    st.download_button(
                        "⬇️ Download Customer PDF",
                        data=pdf_bytes,
                        file_name=make_report_filename("customer", "pdf"),
                        mime="application/pdf",
                        use_container_width=True,
                        key="download_customer_pdf",
                    )
                except Exception as e:
                    st.error(f"Customer PDF generation failed: {e}")
        with customer_col2:
            if st.button("📊 Generate Customer CSV", use_container_width=True):
                try:
                    csv_bytes = dataframe_to_csv_bytes(generate_customer_csv(customer, result))
                    st.download_button(
                        "⬇️ Download Customer CSV",
                        data=csv_bytes,
                        file_name=make_report_filename("customer", "csv"),
                        mime="text/csv",
                        use_container_width=True,
                        key="download_customer_csv",
                    )
                except Exception as e:
                    st.error(f"Customer CSV generation failed: {e}")
    else:
        st.info("Run a prediction in the 🔍 Live Prediction tab first. The latest customer result will appear here.")

    st.markdown("---")
    st.caption("ℹ️ Reports are generated in memory and are not permanently stored on the server. Predictions are risk indicators and should be verified through appropriate field investigation.")
